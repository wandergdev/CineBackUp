...
import { PermissionData } from "@/db/models/Policy/types/PermissionData";
...
function permissionFormat(
  ...
  const removeDuplicates = (originalArray: string[], itemsToRemove: string[]) =>
    _.pullAll(_.uniq(originalArray), _.uniq(itemsToRemove));
  ...
  const currentRoleDataPermission = dataAllowPermissions.concat(
    dataDenyPermissions,
  );

  const allRolesDataPermission = currentRoleDataPermission.concat(
    permission.data,
  );
  permission.data = _.uniqWith(allRolesDataPermission, _.isEqual);
...
function handleDataPermission(
  ...
  const filteredDataPermission: DataPermission[] = [];

  const allowedDataPermission = dataPermission.filter(data => data.isAllowed);
  const notAllowedDataPermission = dataPermission.filter(
    data => !data.isAllowed,
  );

  notAllowedDataPermission.forEach(notAllowedPermission => {
    const isAllowed = allowedDataPermission.some(
      allowedPermission => allowedPermission.data === notAllowedPermission.data,
    );
    if (!isAllowed) {
      filteredDataPermission.push(notAllowedPermission);
    }
  });

  for (const permission of filteredDataPermission) {
...
function roleCanAccessResource(req: Request, permissions: Permission) {
  ...
  const data = handleDataPermission(req, dataPermission, method);
  return data;
}

/**
 *
 * @param customPermissionName Specific permisson to check
 * @param permissions All user permissions source and data.
 * @param req User request
 */
function roleAllowsCustomPermission(
  ...
  const isPlainCustom = permissions.custom.includes(customPermissionName);
  if (isPlainCustom) return true;

  const { method } = req;
  const permissionsToResource = permissions.path.filter(permission =>
    permission.resource.includes(customPermissionName),
  );

  const dataPermission = _.filter(permissions.data, {
    resource: customPermissionName,
    action: requestMethodMapper[method],
  });
...
export function hasCustomPermission(permission: string) {
  return async (req: Request, res: Response, next: Function) => {
    if (req["canModify"]) {
      ...
      const canAccess = roleAllowsCustomPermission(
        permission,
        permissions,
        req,
      );
